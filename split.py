...
FILENAME_FLAG_HELP = f"""{bold}{cyan}Filename to split: {grey}{ FILENAME_FLAG }{end_color}{end_color}"""
BYTES_FLAG_HELP = f"""{bold}{green}segment size in bytes, KiloBytes, MegaBytes or GigaBytes: {grey}{BYTES_FLAG} {cyan}- {grey}{KBYTES_FLAG} {cyan}- {grey}{MBYTES_FLAG} {cyan}- {grey}{GBYTES_FLAG} {end_color}  """
HELP_FLAG_HELP = f"""{bold}{pink}-optional- {bold}{warning}intructions for script:  {grey}{HELP_FLAG}{end_color}"""
EXAMPLE_HELP = f""" {bold}{highlight_grey}example:{end_color} {warning}python {end_color}split.py {grey}{FILENAME_FLAG} {cyan}fileToSplit.pt {grey}{BYTES_FLAG} {green}50000000 """
OUTPUT_FILENAME_HELP = f"""{bold}{pink}-optional- {warning} choose a name for output chunk files: {grey}{OUTPUT_FILENAME_FLAG}{end_color} """
...
#we reduce by one here because name of the script itself counts as an argument.
num_passed_args = (len(script_args)-1)
num_flag_values = num_passed_args - flags_count

if  num_flag_values != flags_count:
    print(f"{fail} Some flags don't have a value. {end_color}")
    sys.exit()

num_size_flags = sum("--size" in arg for arg in script_args)
if  num_size_flags>=2:
